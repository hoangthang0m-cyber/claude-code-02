...
- [x] 13.3 Chạy Migration Plan (production `hem-manager`, 2026-09-08). — ✅ Đã chạy đủ runbook `docs/team-activity-calendar-deploy.md`: `deploy --only firestore:rules` (chỉ **thêm** block lịch, rules CPT nguyên vẹn) + `firestore:indexes` (7 index → **READY**); `seed:calendar-settings` (`config/calendarSettings`), `seed:calendars` (4 lịch dùng chung: `default_goals` managerOnly + campaigns/content/ads everyone), `backfill:members` (4 member từ `users` + lịch cá nhân). Merge `feat/team-activity-calendar` → `main` (`36d45db` + fixup `bab4b6d`) → Vercel deploy. **FCM bỏ qua v1** (answer #3). ⚠️ Còn 1 lịch "thắng" + 1 `calendarItem` cũ trong prod từ phiên test trước (không do build này tạo — để Trưởng phòng quyết xoá/giữ).
...
**Trạng thái nhóm 13:** 13.1 / 13.3 / 13.4 / 13.5 xong — tính năng **đã lên production** (`main` @ `bab4b6d`, rules + 7 index + seed lịch/member đã deploy vào `hem-manager`). Chỉ còn **13.2 Playwright** chờ Trưởng phòng chốt (dựng hay chấp nhận phủ nhiều tầng). Task 1.2 (deploy indexes) đã xong trong bước này.
