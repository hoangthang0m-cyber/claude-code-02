...
- Thư viện thêm: `date-fns`, `rrule`, `@firebase/rules-unit-testing` (dev).
  ...
  `Calendar`.
- `notifications`: dùng lại module `src/modules/notifications` ở mức có thể; chi tiết
  chốt ở nhóm 9–10.
...
- [ ] 9.1 Ô chọn người đảm nhận (`Command` + multi-select) lấy từ `members` active, đánh dấu một người phụ trách chính, mặc định người đầu tiên; verify không thêm được tên ngoài danh bạ.
- [ ] 9.2 Lưu `assigneeIds` + `primaryAssigneeId`; verify đổi phụ trách chính và gỡ hết người đảm nhận lưu đúng.
- [ ] 9.3 Hiển thị trên mục: avatar người phụ trách chính + "+N"; popover liệt kê đầy đủ, phụ trách chính đứng đầu có nhãn; verify với mục 3 người.
- [ ] 9.4 Bộ lọc "Người đảm nhận" (multi-select) áp cho mọi khung nhìn, bền vững khi đổi khung nhìn/điều hướng trong phiên; verify lọc 1 người và 2 người (OR).
- [ ] 9.5 Lối tắt "Việc của tôi" đặt bộ lọc về người đang đăng nhập; verify bật chỉ còn mục mình đảm nhận, tắt trở lại bình thường.
- [ ] 9.6 Quyền chỉnh sửa gắn với người đảm nhận: staff là người đảm nhận sửa/hoàn thành được mục do người khác tạo; verify ở client và rules unit test (nối §1.3).

## 10. Nhắc nhở (capability `calendar-reminders`)

- [ ] 10.1 Kế thừa `defaultReminders` của lịch con khi tạo mục; đổi mặc định không ảnh hưởng mục cũ; verify tạo mục trong lịch có mặc định "30' trước" thì mục có sẵn mốc đó.
- [ ] 10.2 UI thêm/sửa/xoá mốc nhắc theo mục (offset phút/giờ/ngày/tuần + kênh inapp/push), tối đa 5; verify vượt 5 bị chặn; xoá hết = không nhắc kể cả lịch có mặc định.
- [ ] 10.3 Cloud Function trigger `onWrite calendarItems` cho mục **không lặp**: tính lại `dueReminders` `pending` (`sendAt = startAt - offset`, `recipientUids = assigneeIds ∪ createdBy`), bỏ qua `sendAt` quá khứ; verify tạo mục sinh đúng bản ghi; đổi `startAt` dời `sendAt`.
- [ ] 10.4 Scheduled Function 15' cho mục **lặp**: giãn lần hiện trong 36h tới, upsert `dueReminders` idempotent theo khoá `${itemId}_${occurrenceKey}_${offset}`; verify chạy 2 lần không tạo trùng.
- [ ] 10.5 Scheduled Function 1' gửi: `dueReminders where status == pending and sendAt <= now` → tạo `notifications/{uid}/items` cho từng người nhận, gửi FCM nếu `channel == push` và có token, set `status = sent`; verify inapp luôn tạo, push chỉ khi có token, không lỗi khi thiếu quyền push.
- [ ] 10.6 Snooze: nút "bỏ nhắc 5'/30'/1h" tạo `dueReminders` mới; nhắc chưa xử lý vẫn nằm ở danh sách chưa đọc sau khi mục đã bắt đầu; verify cả hai.
- [ ] 10.7 Đăng ký FCM token vào `fcmTokens/{uid}/tokens` + luồng xin quyền thông báo; verify token lưu, thu hồi quyền không làm hỏng gửi inapp.
- [ ] 10.8 Chuông in-app: badge chưa đọc, danh sách, đánh dấu đã đọc, bấm mở đúng mục trên lịch; verify điều hướng tới mục.

## 11. Đăng nhập & phân quyền (capability `calendar-access-control`)

- [ ] 11.1 Cổng xác thực: route lịch yêu cầu Firebase Auth, chưa đăng nhập chuyển tới màn hình đăng nhập rồi quay lại đúng URL; verify deep-link sau đăng nhập.
- [ ] 11.2 Chặn tài khoản không thuộc `members` active: hiển thị màn hình "không có quyền"; verify với user hợp lệ nhưng không phải thành viên phòng.
- [ ] 11.3 Đọc vai trò từ `members/{uid}.role`, cung cấp qua context `usePermissions()`; ẩn/hiện các nút theo ma trận `design.md` §6; verify manager thấy đủ, staff bị giới hạn đúng.
- [ ] 11.4 Lịch `managerOnly`: staff không chọn được khi tạo mục, ghi trực tiếp bị rules chặn, vẫn xem được chế độ chỉ đọc; verify "Mục tiêu phòng".
- [ ] 11.5 Bộ test rules end-to-end phủ toàn bộ ma trận (đọc mọi lịch, tạo theo `writeScope`, sửa/xoá theo creator/assignee/manager, staff không xoá cứng); verify `firebase emulators:exec` chạy suite xanh.
