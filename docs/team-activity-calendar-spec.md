...
**⚠️ Lệch / cần chốt (Requirement "Thông báo khi được giao hoặc gỡ khỏi mục"):** capability `item-assignees` có Requirement thông báo in-app khi thêm/gỡ người đảm nhận (3 Scenario) nhưng **Mục D nhóm 9 KHÔNG có task cho nó**; hạ tầng thông báo (giao `dueReminders` → `notifications`, chuông in-app) nằm ở nhóm 10 (10.5 / 10.8), và spec ghi rõ "`notifications`: chi tiết chốt ở nhóm 9–10". Đề xuất: làm phần thông báo thêm/gỡ + test 3 Scenario **trong nhóm 10** cùng hạ tầng gửi. Cần Trưởng phòng chốt: (a) dùng lại collection phẳng `notifications` của CPT (phải thêm 2 giá trị enum vào `src/lib/domain/enums.ts` — chạm domain CPT), hay (b) một collection thông báo riêng của lịch, hay (c) `notifications/{uid}/items` như Mục C §1.
