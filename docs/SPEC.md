...
- [ ] 3.1 API tạo hạng mục (chỉ `code` + `project_id` bắt buộc); verify tạo ở trạng thái `chua_bat_dau`
- [ ] 3.2 API cập nhật từng trường hạng mục; verify lưu độc lập kèm audit updated_at/actor
...
- [ ] 4.1 State machine 7 trạng thái + các bước hợp lệ (mục 5.3); verify unit test mọi chuyển hợp lệ/không hợp lệ
...
- [ ] 4.6 API `da_duyet` → `da_len_ads` khi có AdsBinding active hoặc manager xác nhận thủ công; verify cả hai đường
- [ ] 4.7 Ghi StatusHistory mọi lần chuyển (from/to, actor, reason); verify tab lịch sử đúng thứ tự thời gian
- [ ] 4.8 Computed `is_overdue` (mục 6.7); verify test các mốc biên (đúng hạn, quá hạn, đã lên ads, đổi deadline)
...
- [ ] 7.1 Kênh realtime theo "phòng" `project_id` (WebSocket/SSE) + fallback polling 10–15s; verify bảng nội dung cập nhật khi người khác đổi trạng thái, tự kết nối lại khi mất kết nối
